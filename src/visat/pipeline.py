"""Build every app artifact into data/app/ (tracked in git, read by Streamlit Cloud).
    ...
    uv run python -m visat.pipeline --source demo      # synthetic, labelled DEMO
    uv run python -m visat.pipeline --source frozen    # real Earth Engine / OSM export
"""
...
from visat import config, exposure, heat_neutral, live, model, news, optimize, validation
from visat.features import add_focal_features, grid_array
from visat.scenarios import Engine, validity_matrix
...
def load(source: str) -> dict:
    if source == "demo":
        from visat import demo_data
        return demo_data.build()
    from visat import frozen
    return frozen.load()
...
def run(source: str) -> dict:
    ...
    for b in config.BUDGET_PRESETS_CR:
        ...
        ours = optimize.summarise(engine, ours_sel, "VISAT plan")
