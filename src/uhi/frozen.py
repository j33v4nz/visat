...
from uhi import config, validation
...
def load() -> dict:
    ...
    if missing:
        raise FileNotFoundError(f"Missing {missing} in data/frozen — run python -m uhi.gee_export first")
    ...
    if (f / "osm.parquet").exists():
        ...
        warnings.warn("No osm.parquet — using proxies (no schools/markets/canals). Run uhi.osm_features.")
