...
**Two corrections below, verified directly against the event site's own source** (`chsrikar/hackme-26` on GitHub — `src/pages/RubricPage.jsx`, the actual React component, not the rendered page, since the site is a client-rendered SPA a normal fetch can't read text from):
...
| **Working Execution & Live Demo** | 20% | Functioning prototype during the pitch; handles edge cases without crashing; test passes; reproducible deployment | **The main risk (§3–4).** Mitigated by data frozen by 2 PM, cached ₹1/10/50 crore presets, a fallback for every live item, pre-drawn project sites, CI, a backup recording, and warming the app up before the slot. UI quality is judged here, not as a separate category. |
...
| **Presentation Craft & Jury Q&A** | 10% | Clarity, time adherence, equal participation, crisp Q&A answers | Each member presents one screen and owns set Q&A topics (PLAN.md §11). Rehearse for the real 5+2 minute window (below), not 3 minutes. |

**The real pitch format is a 5-minute pitch + 2-minute jury Q&A, not the 3-minute demo PLAN.md §10 currently scripts.** Confirmed from the same rubric component: "Functioning prototype during the **5-minute team pitch**" and "Crisp, confident responses... during **2 min Q&A**." PLAN.md's demo script and every "3-minute demo" mention (§10, and the Public Reaction Preview risk-register note) should be rewritten for the actual 5-minute window before the final rehearsal — there's 2 extra minutes to use, best spent slowing down on the Proof screen and giving the Check-a-Project moment room to land, not compressing everything to fit an assumed shorter slot.
...
| Tier 3 **Public Reaction Preview** (simulated personas) is read as a gimmick or as "prompt generation", or quotes invented numbers | Medium | Could cost innovation or credibility points | Keep it **out of the 5-minute pitch** and show it only in Q&A. Personas come from aggregate data tables, never real people, and are labelled "simulated". Schema-validated JSON, a number guard, precomputed cache. It never changes a °C or ₹ figure. It is the **first thing cut in Tier 3** (PLAN.md §4b) | M4 + M3 |
