...
Four screens, one question each: **① Where is heat dangerous today? → ② What should we do with ₹? → ③ Will this project make it hotter? → ④ Can we trust it? · Ward Card**. Built for Kochi Municipal Corporation's climate cell (C-HED) and ward councillors.
...
```bash
uv run python -m visat.pipeline --source demo     # synthetic Kochi, labelled DEMO in the app
# real data (M1): Earth Engine + OSM → data/frozen → data/app
uv sync --all-groups --extra pipeline
uv run earthengine authenticate
uv run python -m visat.gee_export --project <gee-cloud-project>
...
| M3 | App (4 screens, live strip, Check-a-Project UI, Ward Card, deploy) |
